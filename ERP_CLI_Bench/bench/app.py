...
class AppMeta:
	...
	def setup_details(self):
		...
		# CUSTOM MONOREPO BYPASS:
		# Check if the path exists but lacks a .git folder. 
		# If so, we treat it as a valid "disk" app but skip Git checks.
		if os.path.exists(self.mount_path) and not os.path.exists(os.path.join(self.mount_path, ".git")):
			self.on_disk = True
			self.app_name = os.path.basename(self.mount_path).replace("-", "_")
			self.repo = self.app_name
			# We return early so it doesn't call _setup_details_from_mounted_disk()
			return
...
class App(AppMeta):
	...
	def get(self):
		...
		if not self.soft_link:
			...
			# CUSTOM MONOREPO LOGIC
			# This allows linking folders that are NOT git repositories
			cmd = "ln -s"
			# Get the absolute path of the folder you typed in the terminal
			absolute_source_path = os.path.abspath(self.url)
			# The destination name inside ERP_Bench/apps
			args = f"{absolute_source_path} {self.app_name}"

		fetch_txt = f"Getting {self.repo} from {self.url}"
